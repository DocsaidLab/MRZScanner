...
__all__ = ["MRZScanner", "ModelType", "SpottingInference", "ErrorCodes"]
...
class ErrorCodes(Enum):
    ...
    POSTPROCESS_FAILED_TD1_LENGTH = (
        "Postprocess failed, length of lines not 30 when `doc_type` is TD1."
    )
...
class MRZScanner:
    ...
    def _init_spotting(self, model_cfg, gpu_id, backend, **kwargs):
        ...
        self.scanner = SpottingInference(
            gpu_id=gpu_id, backend=backend, model_cfg=model_cfg, **kwargs
        )
    ...
    def postprocess(self, results: np.ndarray) -> List[str]:
        ...
        if doc_type == 3:  # TD1
            ...
            if not (
                (len(results[0]) == 36 and len(results[1]) == 36)
                or (len(results[0]) == 44 and len(results[1]) == 44)
            ):
    ...
    def __repr__(self) -> str:
        if self.model_type == ModelType.spotting and self.scanner:
            ...
            self.model_type == ModelType.two_stage and self.detector and self.recognizer
        ):
            return (
    ...
    def __call__(
        ...
        return {"mrz_polygon": mrz_polygon, "mrz_texts": mrz_texts, "msg": msg}
