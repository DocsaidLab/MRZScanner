...
class MIDV2020BenchmarkDataset:

    def __init__(
        self,
        root: str = "/data/Dataset/MIDV2020",
        return_tensor: bool = False
    ):
        self.root = Path(root)
        self.return_tensor = return_tensor
        ...
        for target, val in gt_codebook.items():
            data = cb.load_json(
                self.root / f'dataset/templates/annotations/{target}.json')
            for d in data['_via_img_metadata'].values():
                for region in d['regions']:
                    if region['region_attributes']['field_name'] == 'mrz_line0':
                        mrz1 = region['region_attributes']['value']
        ...
        for f in cb.Tqdm(cb.get_files(self.root / 'dataset/images/', suffix=['.jpg'])):
            if 'aze_passport' in str(f):
                target = 'aze_passport'
            elif 'grc_passport' in str(f):
                target = 'grc_passport'
